...
import { Headline } from "../ui/Headline";interface Item {
  title?: string;
  description?: string;
  icon?: any;
  ...
  const { title = "", subtitle = "", highlight = "", classes = {} } = props;  const stepsData = {
    items: [
      {
  ...
  };  const { items } = stepsData;  return (
    <div class="max-w-7xl bg-[url('/images/44.jpg')] border-b border-gray-300 dark:border-slate-700">
      <section
        id="roadmap"
        class="mx-auto max-w-5xl scroll-mt-16 px-4 py-10 md:py-12 lg:py-16 sm:px-6 lg:px-8"
      >
        <div class="bg-white/70  px-3 py-6 md:p-6 rounded-lg shadow-lg">
          <Headline title={title} subtitle={subtitle} highlight={highlight} classes={classes?.headline} />      <div class="container text-sm">
        <div class="flex">
          <div class="w-full">
            <div class="road-map-main">
              {items.map(({ title, description, icon: Icon }, index) => (
                ...
                >
                  <div
                    class={twMerge(
                      "road-map-circle w-[200px] h-[200px] rounded-full border-4 border-transparent",
                      index % 2 === 0
                  ...
                  <div
                    class={twMerge(
                      "road-map-card w-1/3 !bg-teal-400 p-5 z-10 rounded-md",
